...
class PersianCalendarPicker extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    let newMonthYear = {};
    if (!jMoment.utc(prevProps.initialDate).isSame(this.props.initialDate, 'day')) {
      newMonthYear = this.updateMonthYear(this.props.initialDate);
      doStateUpdate = true;
    }

    let selectedDateRanges = {};
    if ((this.props.selectedStartDate && !jMoment.utc(prevState.selectedStartDate).isSame(this.props.selectedStartDate, 'day')) || (this.props.selectedEndDate && !jMoment.utc(prevState.selectedEndDate).isSame(this.props.selectedEndDate, 'day'))) {
  ...
  updateScaledStyles(props) {
    const { isRTL, scaleFactor, selectedDays, selectedDayColor, selectedDayTextColor, todayBackgroundColor, width, height, dayShape } = props;
  ...
  handleOnPressDay(day) {
    ...
    const date = jMoment.utc().jYear(currentYear).jMonth(currentMonth).jDate(day);
  ...
  handleOnPressPrevious() {
    ...
    this.props.onMonthChange && this.props.onMonthChange(jMoment.utc().jYear(currentYear).jMonth(currentMonth).jDate(1));
  }
  ...
  render() {
    ...
    const { allowRangeSelection, startFromMonday, initialDate, minDate, maxDate, weekdays, months, previousTitle, nextTitle, textStyle, todayTextStyle, selectedDays, selectedDayStyle, selectedRangeStartStyle, selectedRangeStyle, selectedRangeEndStyle, disabledDates, minRangeDuration, maxRangeDuration, swipeConfig, customDatesStyles, enableDateChange, headingLevel } = this.props;
    ...
    if (allowRangeSelection && minRangeDuration) {
      if (Array.isArray(minRangeDuration)) {
        minRangeDuration.map((minRangeDuration) => {
          const thisDate = jMoment.utc(minRangeDuration.date);
          // thisDate.set({'hour': 0, 'minute': 0, 'second': 0, 'millisecond': 0});
    ...
    if (allowRangeSelection && maxRangeDuration) {
      if (Array.isArray(maxRangeDuration)) {
        maxRangeDuration.map((maxRangeDuration) => {
          const thisDate = jMoment.utc(maxRangeDuration.date);
          // thisDate.set({'hour': 0, 'minute': 0, 'second': 0, 'millisecond': 0});
    ...
    return (
      <Swiper onSwipe={(direction) => this.props.enableSwipe && this.onSwipe(direction)} config={{ ..._swipeConfig, ...swipeConfig }}>
        <View syles={styles.calendar}>
          <HeaderControls styles={styles} currentMonth={currentMonth} currentYear={currentYear} initialDate={jMoment.utc(initialDate)} onPressPrevious={this.handleOnPressPrevious} onPressNext={this.handleOnPressNext} months={months} previousTitle={previousTitle} nextTitle={nextTitle} textStyle={textStyle} headingLevel={headingLevel} />

          <Weekdays styles={styles} startFromMonday={startFromMonday} weekdays={weekdays} textStyle={textStyle} />
          <DaysGridView
            ...
            selectedStartDate={selectedStartDate && jMoment.utc(selectedStartDate)}
